...
\begin{document}
	...
	\begin{frame}{马氏链中的环流}
		...
		\end{block}
		% \begin{block}{是否可使用环表示上述马氏过程？}
		% 	考虑马氏链 $(\mathit{X}_i)_{i\in \mathbb{N}}$ 前 $n$ 步，并且令 $\mathit{X}_{i+1} = \mathit{X}_1$，则$\left(X_1, X_2, \dots, X_n, X_{n+1}\right)$ 构成了一个回路，按照环产生的次序，会有一个环的序列 $\{\xi_1, \xi_2, \dots, \xi_j\}$，是否可以用该序列唯一的表示一个马氏链？
		% \end{block}
	\end{frame}
